...
public class TypeVisitor implements T_Visitor {
	...
	public Types.Type visit(CallExpr ast) {
		...
		for (Expr expression : ast.args) {
			...
			Types.Type t2 = temp.removeFirst();
	...
	public Types.Type visit(NewArrayExpr ast) {
		...
		for (Expr e : ast.dimensions) {
			...
			if (t.coerceTo(new Types.INT())) {
				...
				System.err
						.println("ERROR incompatible types: int required, but "
								+ t.toString() + "found");
				System.exit(0);
			}
		}
	...
	public Types.Type visit(Program ast) {
		...
		LinkedList<Types.CLASS> classesTemp = new LinkedList<Types.CLASS>();
		...
		for (ClassDecl classDecl : ast.classes) {
			...
			Types.CLASS temp =  classesTemp.removeFirst(); //(Types.CLASS) scopeTab.get(classDecl.name);
			...
			for (int i = 0; i < classDecl.methods.size(); i++) {
				...
				/*if(classDecl.methods.get(i).name.equals("main"))
					curr.result = new Types.VOID();
				else
				...
				scopeTab.put(classDecl.methods.get(i).name, curr);
		...
		for (ClassDecl classDecl : ast.classes) {
			...
			for (Types.FIELD f : myClass.methods) {
				temp.methods.put(f, f.name);
				((Types.FUNCTION)f.type).self = temp;
			}
